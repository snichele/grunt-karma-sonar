...
module.exports = function (grunt) {
  ...
  grunt.registerMultiTask('karma_sonar', 'Grunt plugin for integrating karmma reports with sonar', function () {
    ...
    function getSonarArguments(data, options) {
      ...
      var analysisMode =(data.analysis && data.analysis.mode) ? data.analysis.mode : 'analysis';

      if(data.analysis && data.analysis.mode) {
        args.push('-Dsonar.issuesReport.console.enable=true');
        args.push('-Dsonar.issuesReport.html.enable=true');
      }

      args.push('-Dsonar.sourceEncoding=' + 'UTF-8');

      args.push('-Dsonar.language=' + options.language);
      args.push('-Dsonar.analysis.mode=' + analysisMode);
